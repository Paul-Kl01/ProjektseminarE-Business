 ...
 */
class wave {
  constructor(entities, mapType = 0) {
    //default mapType = 0
  ...
  initialiseEnemies(enemyStrength = 1) {
    //Typ 1 als default Enemy, da Typ 0 = Boss
    ...
    if (this.enemyGroup > 0 && this.enemyGroupCoolDown == 0) {
      ...
      if (this.enemySpawnCooldown > 0) {
        ...
        this.enemySpawnCooldown = this.getRndInteger(
          this.currentMinCooldown,
  ...
  nextWave() {
    //Klassenvariablen für die nächste Wave vorbereiten
    this.currentWave++;
    ...
    if (this.currentWave % this.currentSkaling == 0) {
      ...
      this.enemySettings[3][4] = Math.floor(
        Math.pow(this.enemySettings[3][4], 1.2)
      );
      //Bei der DefaultMap soll schneller Enemy ebenfalls hochskaliert werden
      if (this.mapType == 1) {
        this.enemySettings[2][4] =
          Math.floor(Math.pow(this.enemySettings[2][4] + 1, 1.2)) *
          this.mapType;
      }
    ...
    if (this.currentMaxCooldown > this.maxCooldown) {
      this.currentMaxCooldown =
        this.currentMaxCooldown - this.cooldownDecrement;
    }
    if (this.currentMinCooldown > this.minCooldown) {
      this.currentMinCooldown =
        this.currentMinCooldown - this.cooldownDecrement;
    }
    this.enemySpawnCooldown = this.getRndInteger(
      this.currentMinCooldown,
      this.currentMaxCooldown
...
/*
 * Verwaltung der Entitäten im Spiel & Spiellogik
 * @author Anika
 *
 */
class Entities {
    ...
    for (let j = 0; j < this.towerList.length; j++) {
      for (let k = 0; k < this.towerList[j].particleList.length; k++) {
        if (
          ...
          this.towerList[j].particleList[k].enemy.dead == true
        )
          continue;
        var bool = this.detectCollision(
          this.towerList[j].particleList[k].x,
        ...
        if (bool == true) {
          ...
          this.towerList[j].particleList[k].enemy.hit(
            this.towerList[j].particleList[k].damage
          );
          this.towerList[j].particleList[k].flag = true;
    ...
    for (let i = -1; i < this.waypoints.length - 1; i++) {
      ...
      for (let j = start; j < finish; j++) {
        ...
        if (change_x == true)
          ...
          else bool = this.detectCollision(j, y1, 35, x, y, radius);
